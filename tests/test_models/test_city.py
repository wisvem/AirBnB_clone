...
class Test_City_class(unittest.TestCase):
    ...
    def test_storage_state_class_in_object_dict(self):
        ...
        for objid in all_objs:
            pass
        mystr = my_city.__class__.__name__+'.'+my_city.id
    ...
    def test_reload_function_for_state_class(self):
        ...
        filename = "file.json"
        my_city = City()
        my_obj = my_city.__class__.__name__ +'.'+my_city.id
        ...
        storage.reload()
        all_obj = storage.all()
        self.assertFalse(my_city == all_obj[my_obj])
        self.assertEqual(my_city.id, all_obj[my_obj].id)
        self.assertEqual(my_city.__class__, all_obj[my_obj].__class__)
        self.assertEqual(my_city.created_at, all_obj[my_obj].created_at)
        self.assertEqual(my_city.updated_at, all_obj[my_obj].updated_at)
        self.assertTrue(len(storage.all()) == 1)
