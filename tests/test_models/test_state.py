...
class Test_Base_model(unittest.TestCase):
    ...
    def test_storage_state_class_in_object_dict(self):
        ...
        for objid in all_objs:
            pass
        mystr = my_state.__class__.__name__+'.'+my_state.id
        self.assertEqual(objid, mystr)
        # test full object
        objid = {mystr: my_state}
    ...
    def test_reload_function_for_state_class(self):
        ...
        filename = "file.json"
        my_state = State()
        my_obj = my_state.__class__.__name__ +'.'+my_state.id
        ...
        storage.reload()
        all_obj = storage.all()
        self.assertFalse(my_state == all_obj[my_obj])
        self.assertEqual(my_state.id, all_obj[my_obj].id)
        self.assertEqual(my_state.__class__, all_obj[my_obj].__class__)
        self.assertEqual(my_state.created_at, all_obj[my_obj].created_at)
        self.assertEqual(my_state.updated_at, all_obj[my_obj].updated_at)
        self.assertTrue(len(storage.all()) == 1)
